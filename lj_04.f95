...
integer, parameter 	:: num=1500,steps=10000000,chunk=1000000
real			:: size = 20,diff,repul,attra,energy,cool=-0.4
real, parameter		:: tstep = 0.00001,rm = 1,e = 10,m = 1,epermass = 10
...
do i = 1, steps ! LOOP TIMESTEPS
	...
	do j = 1, num ! LOOP PARTICLES
		do k = 1, 3 ! LOOP VELOCITY DIMENSIONS
			...
			else ! JUST INCASE OF FUNNY BUSINESS
				new(j,k) = rand()*size
...
! INTERMEDIATE DUCK FILE
	if (i .EQ. 1) then
		open(unit=20, file="bosiduck.dat")
		do j = 1, num
			write(20,*) old(j,:)
		end do
		close(20)
